...
const PackageOptionCard = ({
  ...
  return (
    <div className={`${backgroundColor} rounded-4xl p-5 px-8 relative h-full flex flex-col`}>
      <h3 className={titleClassName}>{title}</h3>
      <p className={`${descriptionClassName} grow`}>{description}</p>
      <div className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 rounded-full ${circleColor} border-2 ${borderColor} w-12 h-12`} />
    </div>
  );
};
